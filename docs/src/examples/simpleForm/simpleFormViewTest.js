...
simpleFormViewSuite.add("spike", assert => {
    ...
    assert.is(root.querySelectorAll("label").length, 8);
    assert.is(root.querySelectorAll("input").length, 7);
    assert.is(root.querySelectorAll("select").length, 1);
    assert.is(root.querySelectorAll("select option").length, 3);
    assert.is(root.querySelectorAll("datalist option").length, 3);
